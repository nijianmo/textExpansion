...
class EncoderRNN(nn.Module):
    ...
    def forward(self, x1, x1_len, x2, x2_len, hidden=None):
        ...
        out1_p, hidden1 = self.gru(x1_emb_p, hidden) # output: (seq_len_1, batch, hidden*n_dir)
        out2_p, hidden2 = self.gru(x2_emb_p, hidden) # output: (seq_len_2, batch, hidden*n_dir)
        ...
        out1 = out1[:, :, :self.hidden_size] + out1[:, : ,self.hidden_size:] # Sum bidirectional outputs (seq_len_1, batch, hidden)
        out2 = out2[:, :, :self.hidden_size] + out2[:, : ,self.hidden_size:] # Sum bidirectional outputs (seq_len_2, batch, hidden)
        hidden = hidden1 + hidden2 # [1, batch, hidden]
...
class AttributeEncoder(nn.Module):
    def __init__(self, attr_size, attr_num, hidden_size, attr_embeddings, n_layers=1, dropout=0.2):
        ...
        self.attr = nn.Linear(self.attr_size * self.attr_num, self.n_layers * self.hidden_size) 
        self.tanh = nn.Tanh()

    def forward(self, input):
        embeddeds = [] # (attr_num, batch, attr_size) = (A,B,K)
        embeddeds.append(self.user_embedding(input[:, 0]))
        embeddeds.append(self.item_embedding(input[:, 1]))
...
class LuongAttnDecoderRNN(nn.Module):
    ...
    def forward(self, input_seq, last_hidden, encoder_out1, encoder_out2, encoder_out3, encoder_out4):
        ...
        :param encoder_out4:
            encoder outputs in shape (A,B,P), P is aspect num
        '''
        ...
        encoder_out4 = encoder_out4.repeat(seq_len,1,1) # [N,B,P]
