...
class DecoderRNN(nn.Module):
    ...
    def forward(self, input_seq, last_hidden, mask=None):
        # Get the embedding of the input sequence
        embedded = self.drop(self.embedding(input_seq))
        rnn_output, hidden = self.gru(embedded, last_hidden) # [layer, batch, hidden]
        output = self.drop(rnn_output)
        output = self.out(output.squeeze(0))
        #output = F.softmax(output)
        return output, hidden

    def init_hidden(self, bsz): # set as 0
