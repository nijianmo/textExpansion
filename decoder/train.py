...
def trainIters(corpus, reverse, n_epoch, learning_rate, batch_size, n_layers, hidden_size, 
    ...
    try:
        training_batches = torch.load(os.path.join(save_dir, 'data', '{}_{}.tar'.format(filename(reverse, 'training_batches'), batch_size)))
        ...
        torch.save(training_batches, os.path.join(save_dir, 'data', '{}_{}.tar'.format(filename(reverse, 'training_batches'), batch_size)))

    # validation/test data
    eval_batch_size = 10
    try:
        val_batches = torch.load(os.path.join(save_dir, 'data', '{}_{}.tar'.format(filename(reverse, 'val_batches'), eval_batch_size)))
    except FileNotFoundError:
        ...
        torch.save(val_batches, os.path.join(save_dir, 'data', '{}_{}.tar'.format(filename(reverse, 'val_batches'), eval_batch_size)))

    try:
        test_batches = torch.load(os.path.join(save_dir, 'data', '{}_{}.tar'.format(filename(reverse, 'test_batches'), eval_batch_size)))
    except FileNotFoundError:
        ...
        torch.save(test_batches, os.path.join(save_dir, 'data', '{}_{}.tar'.format(filename(reverse, 'test_batches'), eval_batch_size)))
